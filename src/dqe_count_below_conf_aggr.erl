-module(dqe_count_below_conf_aggr).
-behaviour(dqe_fun).
...
spec() ->
    [{<<"count_below_conf">>, [metric, integer, time], none, metric},
     {<<"count_below_conf">>, [metric, float, time], none, metric}].

run([Data], S = #state{const = Const, count = Count}) ->
    {mmath_aggr:count_below_conf(Data, Const, Count), S}.

help() ->
    <<"Returns the count of value below a given confidence threashold.">>.
